...
if [ -d "$INSTALL_DIR/.git" ]; then
  ...
  # bun.lock drift from prior `bun install` blocks ff-only pulls. Safe to
  # reset — regenerated by the bun install step below.
  git -C "$INSTALL_DIR" checkout -- bun.lock cli/bun.lock 2>/dev/null || true
