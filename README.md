...
| Path | What | Lives at |
|------|------|----------|
| `ae/` | The `ae` CLI (renamed from `astack`) — Bun + TypeScript | binary: `ae` |
| `web/` | Landing page that serves the install one-liner | https://ara.engineer |
...
```bash
ln -snf "$(pwd)/ae/bin/ae" ~/.bun/bin/ae
ae --version
```

## web — the landing page

Static site. See `web/README.md`.

## Ship it

1. Push this repo to `github.com/Aradotso/ara.engineer`.
2. `cd web && vercel deploy --prod` (team `araso`).
3. Point `ara.engineer` DNS at Vercel.
4. `curl -fsSL https://ara.engineer/install | sh` should install `ae`.
