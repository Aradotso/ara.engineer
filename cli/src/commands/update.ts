...
// Files that normal `aracli` use rewrites in a managed install. Safe to
// silently reset before pulling — either regenerated on demand (bun.lock)
// or treated as per-user state that upstream doesn't track (sauce-list).
const MANAGED_DRIFT_FILES = new Set([
  "cli/bun.lock",
  "cli/package-lock.json",
  "skills/sauce/references/sauce-list.md",
]);

// Returns the list of tracked+modified paths that are NOT in the managed-drift
// allow-list. Untracked files (`??`) are ignored — `git pull` doesn't care
// about them. Empty return → safe to auto-reset and pull.
async function unexpectedChanges(repo: string): Promise<string[]> {
  const r = await run(["git", "status", "--porcelain"], { cwd: repo });
  const paths: string[] = [];
  for (const line of r.stdout.split("\n")) {
    if (!line.trim()) continue;
    // Porcelain format: "XY path". "??" = untracked (ignored here).
